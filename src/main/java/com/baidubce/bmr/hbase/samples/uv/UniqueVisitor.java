...
public class UniqueVisitor {
  ...
  private static Job configureJob(final Configuration conf, final String[] args) 
    ...
    Job job = Job.getInstance(conf, "UniqueVisitor");
    job.setJarByClass(UniqueVisitor.class);

    Scan scan = new Scan();
    scan.setBatch(0);
    scan.setCaching(500);
    scan.setCacheBlocks(false);
    scan.addColumn(TableInfo.COLUMN_FAMILY, TableInfo.COLUMN_REMOTE_ADDR);
    scan.addColumn(TableInfo.COLUMN_FAMILY, TableInfo.COLUMN_TIME_LOCAL);
    TableMapReduceUtil.initTableMapperJob(tableName, scan, UniqueVisitorMapper.class, 
  ...
  /**
   * Main entrance for UniqueVisitor.
   * 
   */
  public static void main(String[] args) {
    ...
    if (otherArgs.length != 2) {
      System.err.println("Usage: UniqueVisitor <source table name> <target path>");
      System.exit(2);
    }
